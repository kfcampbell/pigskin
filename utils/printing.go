...
func FormatBiggestWin(biggestWin responses.FantasyGame, difference float32) string {
	if IsHomeWinning(biggestWin) > -1 {
		return fmt.Sprintf("- **Biggest win**: _%v_ over _%v_, %.2f-%.2f (difference of %.2f points).",
...
func FormatClosestGame(closestGame responses.FantasyGame, difference float32) string {
	if IsHomeWinning(closestGame) > 0 {
		return fmt.Sprintf("- **Closest game**: _%v_ over %v, %.2f-%.2f (difference of %.2f points).", closestGame.Home.Name, closestGame.Away.Name, closestGame.HomeScore.Score.Value, closestGame.AwayScore.Score.Value, difference)
	}

	return fmt.Sprintf("- **Closest game**: _%v_ over %v, %.2f-%.2f (difference of %.2f points).", closestGame.Away.Name, closestGame.Home.Name, closestGame.AwayScore.Score.Value, closestGame.HomeScore.Score.Value, difference)
}
...
func FormatTopScorers(topScorers *Scorers) string {
	...
	for i := 0; i < topScorers.GetLength(); i++ {
		formatted += fmt.Sprintf("_%v_ (%.2f points), ", topScorers.GetScorers()[i].Team.Name, topScorers.GetScorers()[i].Score)
	}
...
func FormatBottomScorers(bottomScorers *Scorers) string {
	...
	for i := 0; i < bottomScorers.GetLength(); i++ {
		formatted += fmt.Sprintf("_%v_ (%.2f points), ", bottomScorers.GetScorers()[i].Team.Name, bottomScorers.GetScorers()[i].Score)
	}
	formatted = strings.TrimSuffix(formatted, ", ")
